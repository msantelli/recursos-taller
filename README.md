...
- **Tipografía como argumento.** Dos webfonts de Google Fonts con pilas de reserva del sistema: **Literata** (serif editorial) para el cuerpo de lectura y **IBM Plex Mono** (voz de máquina) para etiquetas, metadatos, términos técnicos y código. El contraste entre las dos voces es el tema de la página hecho forma.
- **Dos tintas.** La versión inicial usaba una sola tinta de acento (azul profundo, `#17497a`); a pedido del docente durante la clase («ponele picante») se sumó una segunda: un **rojo rúbrica** (`#b23a26`), el color con que los manuscritos marcaban títulos e iniciales — que además es el coral del afiche del taller. El azul quedó para los enlaces; el rojo para cejas, capitulares, el cursor de la terminal y los hovers. La cinta de dos colores arriba de la página es la firma de esa convivencia.
- **La lámina del hero** es *El impresor* de Jost Amman (Ständebuch, 1568), dominio público, descargada de [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Buchdrucker-1568.png) y autoalojada en `imagenes/`. En modo claro se funde con el papel (`mix-blend-mode: multiply`); en modo oscuro se invierte, como un negativo de imprenta.
